...
Public Class Form1
    ...
    Dim fromLength(0) As Double
    Dim toLength(0) As Double
    Dim Lab(0) As Double
    ...
    Private Sub eliminateDup()
        ...
        For i = 0 To NumVertexFrom - 2
            If getFromAngle(i) = getFromAngle(i + 1) Then
                countFrom += 1
            Else
                ReDim CopyFromX(m)
                CopyFromX(m) = FromX(i)
    ...
    Private Sub VertexMorphingProgress(ByRef steps As Integer)
        ...
        For i = 0 To numofVertex - 1
            MorphX(i) = CopyFromX(i) + (CopyToX(i) - CopyFromX(i)) * steps / TotalStep
            MorphY(i) = CopyFromY(i) + (CopyToY(i) - CopyFromY(i)) * steps / TotalStep
        Next
    ...
    Public Function getFromAngle(ByVal i As Integer) As Double
        Dim x1 As Double
        Dim y1 As Double
        Dim angle As Double
        If i = 0 Then
            x1 = CopyFromX(i) - CopyFromX(numofVertex - 2)
            y1 = CopyFromY(i) - CopyFromY(numofVertex - 2)
        Else
            x1 = CopyFromX(i) - CopyFromX(i - 1)
            y1 = CopyFromY(i) - CopyFromY(i - 1)
        End If
    ...
    Public Function getToAngle(ByVal i As Integer) As Double
        ...
        Dim angle As Double
        If i = 0 Then
            x1 = CopyToX(i) - CopyToX(numofVertex - 1)
            y1 = CopyToY(i) - CopyToY(numofVertex - 1)
        Else
            x1 = CopyToX(i) - CopyToX(i - 1)
            y1 = CopyToY(i) - CopyToY(i - 1)
        End If
        angle = Math.Atan2(y1, x1)
        'regulateAngle(angle)
        Return angle
    ...
    Private Sub EdgeMorphingProgress(ByRef steps As Integer)
        ...
        Console.WriteLine(t)
        Dim MorphAngle(numofVertex - 1) As Double
        Dim fromAngle(numofVertex - 1) As Double
        Dim toAngle(numofVertex - 1) As Double

        Dim s(numofVertex - 1) As Double
        ReDim fromLength(numofVertex - 1)
        ReDim toLength(numofVertex - 1)
        ...
        'initialize all morphangle and morphx and morphy
        For i = 0 To numofVertex - 1
            fromAngle(i) = getFromAngle(i)
            toAngle(i) = getToAngle(i)
            'MorphAngle(i) = fromAngle(i)
            If i > 0 Then
                fromX1 = CopyFromX(i) - CopyFromX(i - 1)
                fromY1 = CopyFromY(i) - CopyFromY(i - 1)
                ToX1 = CopyToX(i) - CopyToX(i - 1)
                ToY1 = CopyToY(i) - CopyToY(i - 1)
                fromLength(i) = Math.Sqrt(fromX1 * fromX1 + fromY1 * fromY1)
                toLength(i) = Math.Sqrt(ToX1 * ToX1 + ToY1 * ToY1)
            End If
        Next

        fromLength(0) = fromLength(numofVertex - 1)
        toLength(0) = toLength(numofVertex - 1)
        s(0) = 0
        newLength1(0) = 0
        MorphL(0) = 0
        ' end of initialization

        Dim fromAngleChange As Double
        Dim toAngleChange As Double
        Dim newChange As Double

        'below computes new angle changes
        MorphAngle(0) = 0
        For i = 1 To numofVertex - 1
            fromAngleChange = fromAngle(i) - fromAngle(i - 1)
            toAngleChange = toAngle(i) - toAngle(i - 1)
            regulateAngle2(fromAngleChange)
            regulateAngle2(toAngleChange)

            'newChange value is in between -pi to pi
            newChange = fromAngleChange * (1 - t) + toAngleChange * steps / TotalStep
            MorphAngle(i) = MorphAngle(i - 1) + newChange '- Math.PI
            regulateAngle2(MorphAngle(i))
        Next

        'below computes new Length before tweakling
        newLength1(0) = 0
        For i = 1 To numofVertex - 1
            'below are vector lines  to compute new length of i to i-1
            newLength1(i) = fromLength(i) * (1 - t) + toLength(i) * t
        Next
        ...
        For i = 0 To numofVertex - 1
            edge_difference = Math.Abs(fromLength(i) - toLength(i))
        ...
        For i = 0 To numofVertex - 1
            If i = 0 Or i = numofVertex - 1 Then
                MorphX(i) = CopyFromX(i) * (1 - t) + CopyFromX(i) * t
                MorphY(i) = CopyFromY(i) * (1 - t) - CopyFromY(i) * t
            Else
                'use edge tweakling to close the shape
                Dim e As Double = ComputeE(MorphAngle)
                Dim f As Double = ComputeF(MorphAngle)
                Dim g As Double = ComputeG(MorphAngle)
                Dim u As Double = ComputeU(newLength1, MorphAngle)
                Dim v As Double = ComputeV(newLength1, MorphAngle)
                Dim lambda_1 As Double = Computelambda1(e, f, g, u, v)
                Dim lambda_2 As Double = Computelambda2(e, f, g, u, v)
                'Console.WriteLine(lambda_1)
                'Console.WriteLine(lambda_2)

                regulateAngle(MorphAngle(i))
                s(i) = -0.5 * Lab(i) * Lab(i) * (lambda_1 * Math.Cos(MorphAngle(i)) + lambda_2 * Math.Sin(MorphAngle(i)))
                ' Console.WriteLine(s(i))
                'end of normalization in edge tweakling
                MorphL(i) = newLength1(i) + s(i)

                MorphX(i) = MorphX(i - 1) + MorphL(i) * Math.Cos(MorphAngle(i))
                MorphY(i) = MorphY(i - 1) + MorphL(i) * Math.Sin(MorphAngle(i))
            End If
        Next
    ...
    Private Function ComputeLtol() As Double
        ...
        For i = 0 To numofVertex - 1
            ...
            End If
        Next
        Return 0.00001 * max_edge_difference
    End Function
    ...
    Public Sub CreateBitmapAtRuntime()
        pictureBox1.Size = New Size(1210, 710)
        'morph panel size is 1200, 780
        Me.Controls.Add(pictureBox1)

        Dim flag As New Bitmap(1200, 700)
        Dim flagGraphics As Graphics = Graphics.FromImage(flag)
        Dim red As Integer = 0
        Dim white As Integer = 11
        flagGraphics.FillRectangle(Brushes.White, 0, 0, 1200, 700)
        While white <= 100
            ...
            white += 20
        End While
        flagGraphics.DrawLines(Pens.Blue, FromPoints.ToArray())
        'flagGraphics.DrawLine(Pens.Red, New Point(MorphX(0), MorphY(0)), New Point(MorphX(1), MorphY(1)))
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        'SaveFileDialog1.Title = "Please Select Initial Shape File"
        ...
        'pictureBox1.Image.Save("C:\Users\Xiaohan Wang\Desktop\Data\newImage", System.Drawing.Imaging.ImageFormat.Jpeg)
